...
// Model + effort are env-configurable so you can trade cost against accuracy without editing code.
//
// Benchmarked against a real 2-bedroom plan, scoring whether each room landed in its true
// quadrant (Living upper-right, Kitchen mid-right, Bedroom 2 lower-right, Hall left-of-center):
//
//   sonnet-5 / medium   2/4   $0.034    16s   <- Living Room placed in the wrong half
//   sonnet-5 / high     ---   ---       ---   <- overruns max_tokens, JSON truncates
//   opus-5   / low      4/4   $0.118    35s   <- default
//   opus-5   / medium   4/4   $0.263   108s
//   opus-5   / high     4/4   $0.511   253s
//
// The takeaway: spatial layout accuracy tracks the MODEL, not the effort. Opus at its cheapest
// setting beats Sonnet at any setting, so effort is the right dial for cost and Opus is the
// floor for correctness.
const MODEL = process.env.ANTHROPIC_MODEL || 'claude-opus-5';
// `high` is the quality setting: it produced the cleanest room shapes and adjacency in testing.
// Coverage % was flat across efforts, but shape/topology quality was not — low tends to emit more
// small rooms split apart, which is what creates spurious walls between open-plan spaces.
const FLOOR_PLAN_EFFORT = process.env.FLOOR_PLAN_EFFORT || 'medium';
const STYLE_EFFORT = process.env.STYLE_EFFORT || 'low';
// Product pages are a plain extraction task — Haiku is ~5x cheaper and sufficient.
const PRODUCT_MODEL = process.env.PRODUCT_MODEL || 'claude-haiku-4-5';

// $ per million tokens, for the cost line printed after each call.
const PRICING = {
  'claude-opus-5': { in: 5, out: 25 },
  'claude-sonnet-5': { in: 2, out: 10 }, // introductory pricing through 2026-08-31
  'claude-haiku-4-5': { in: 1, out: 5 },
};

function logUsage(label, response, model = MODEL) {
  const u = response?.usage;
  if (!u) return;
  const price = PRICING[model];
  const cost = price ? (u.input_tokens * price.in + u.output_tokens * price.out) / 1e6 : null;
  console.log(
    `${label}: ${model} in=${u.input_tokens} out=${u.output_tokens}` +
...
/**
 * Builds an Anthropic client from the caller's own API key.
 *
 * The key is supplied per-request by the browser (stored only in the user's localStorage) and is
 * never persisted server-side. A server-held key would mean every visitor to the public demo spends
 * the deployer's money, so `ANTHROPIC_API_KEY` is only honoured for local development.
 */
function clientFor(req) {
  const header = req.get?.('x-anthropic-key') || req.headers?.['x-anthropic-key'];
  const apiKey = (typeof header === 'string' && header.trim()) || process.env.ANTHROPIC_API_KEY;
...
const PRODUCT_PROMPT = `You are given a furniture product page: its visible text, and its main product image.
...
const STYLE_PROMPT = `These are photos of the inside of one apartment. Extract its interior finishes so a 3D model can be styled to match. Judge colors from the well-lit parts of the photos and return average, representative tones (not shadow or glare regions): the flooring material and its dominant + secondary colors, the wall paint color, the baseboard color and approximate height, the window frame color, and the typical window sill/top heights off the floor (0 sill for floor-to-ceiling glass). Return only the structured data.`;

const PROMPT = `This image is an architectural floor plan. Read it the way an architect would, then output the room layout as structured data.
...
app.post('/api/analyze-floor-plan', async (req, res) => {
  ...
  const { imageBase64, mediaType, _model, _effort } = req.body ?? {};
  if (!imageBase64 || typeof imageBase64 !== 'string') {
    res.status(400).json({ error: 'Missing imageBase64 in request body.' });
    return;
  }
  // Per-request overrides exist only for local benchmarking of accuracy vs. cost.
  const model = _model || MODEL;
  const effort = _effort || FLOOR_PLAN_EFFORT;

  try {
    // Streamed with a large max_tokens: thinking tokens share this budget with the response, and
    // a detailed plan yields a lot of polygon points — too small a budget silently truncates the
    // JSON and yields no parsed_output. Streaming also avoids HTTP timeouts on multi-minute runs.
    const stream = client.messages.stream({
      model,
      // Thinking shares this budget with the JSON. A detailed plan at high effort can spend
      // 30k+ tokens reasoning, so keep plenty of headroom or the output truncates mid-object.
      max_tokens: 64000,
      output_config: {
        effort,
    ...
    if (response.stop_reason === 'refusal') {
      res.status(422).json({ error: "Claude declined to analyze this image. Try a clearer floor plan image." });
      return;
    }

    // `parsed_output` is only populated for SDK-native parseable formats (e.g. zodOutputFormat);
    // with a raw JSON schema the payload arrives as ordinary text, so parse it ourselves.
    ...
    if (!parsed) {
      ...
      try {
        ...
        console.error('Unparseable output. stop_reason:', response.stop_reason, 'usage:', response.usage);
        const detail =
          response.stop_reason === 'max_tokens'
            ? 'Claude ran out of output space analyzing this plan. Try a lower-resolution floor plan image.'
            : 'Claude returned a response that could not be parsed as structured room data.';
        res.status(502).json({ error: detail });
        return;
      }
    }

    logUsage(`analyze-floor-plan[${effort}]`, response, model);
    console.log(
      `  → ${parsed.rooms?.length ?? 0} rooms, ${parsed.doors?.length ?? 0} doors, ${parsed.windows?.length ?? 0} windows, ${parsed.fixtures?.length ?? 0} fixtures`,
    );
    res.json(parsed);
  } catch (err) {
...
/** Strips a product page down to readable text, keeping the title/meta and dropping markup. */
...
/** Pulls the most likely main product image URL out of a product page. */
function findProductImage(html, pageUrl) {
  const patterns = [
    /<meta[^>]+property=["']og:image["'][^>]+content=["']([^"']+)["']/i,
    /<meta[^>]+name=["']twitter:image["'][^>]+content=["']([^"']+)["']/i,
    /"hiRes"\s*:\s*"([^"]+)"/i, // Amazon image blocks
    /"large"\s*:\s*"([^"]+)"/i,
...
app.post('/api/product-from-url', async (req, res) => {
  const client = clientFor(req);
  if (!client) {
    res.status(401).json({ error: MISSING_KEY_MESSAGE });
    return;
  ...
  let browser;
  try {
    // Retailers serve their real product content via JavaScript and block plain HTTP clients
    // (measured: Amazon 404s, Wayfair 429s, IKEA returns a generic shell). Rendering the page in
    // a real browser is what actually gets the data — and it lets us screenshot the page so the
    // model can *see* the product exactly as a shopper would.
    ...
    try {
      ...
      await page.waitForTimeout(2500); // let client-rendered product details settle
    } catch {
      res.status(422).json({ error: 'That page took too long to load. Check the link and try again.' });
      return;
    }
    if (status >= 400) {
      res.status(422).json({
        error:
          `The store returned HTTP ${status} for that link — it likely blocks automated access. ` +
          `Try a different retailer, or add the item from the catalog and set its size by hand.`,
      });
      return;
    }
    ...
    // Grab the main product photo for the side panel.
    const imageUrl = findProductImage(html, url);
    if (imageUrl) {
      try {
        const buf = await (await context.request.get(imageUrl)).body();
        ...
        // Non-fatal; the screenshot below still shows the product.
      }
    }

    // A screenshot of the rendered page carries the dimension tables and swatches that the
    // stripped text often loses.
    try {
    ...
    logUsage('product-from-url', response, PRODUCT_MODEL);
    console.log(`  → ${parsed.name} ${parsed.widthIn}x${parsed.depthIn}x${parsed.heightIn}in (found=${parsed.dimensionsFound})`);
    res.json({ ...parsed, imageDataUrl, sourceUrl: url });
  } catch (err) {
...
app.post('/api/extract-style', async (req, res) => {
  ...
  try {
    const stream = client.messages.stream({
      ...
      output_config: {
        effort: STYLE_EFFORT,
        format: { type: 'json_schema', schema: STYLE_SCHEMA },
      },
      messages: [
        {
          role: 'user',
    ...
    if (!parsed) {
      ...
      try {
        ...
        res.status(502).json({ error: 'Claude returned a response that could not be parsed as a style profile.' });
        return;
      }
    }

    logUsage('extract-style', response);
...
app.listen(PORT, () => {
  console.log(`Floor plan AI server listening on http://localhost:${PORT}`);
  console.log(`Model: ${MODEL} (floor plan effort: ${FLOOR_PLAN_EFFORT}, style effort: ${STYLE_EFFORT})`);
  if (!process.env.ANTHROPIC_API_KEY) {
    console.warn('Warning: ANTHROPIC_API_KEY is not set — /api/analyze-floor-plan will return an error until it is.');
  }
});
